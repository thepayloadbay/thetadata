...
def append_results_md(all_trades: list, date_list) -> None:
    ...
    L = [
        ...
        f"| Max credit | {'$'+f'{MAX_NET_CREDIT:.2f}' if MAX_NET_CREDIT is not None else 'none'} |",
        ...
        f"| Kelly sizing | {'on' if ENABLE_KELLY_SIZING else 'off'} |",
        f"| Daily SL | {'$'+str(int(DAILY_SL)) if DAILY_SL is not None else 'off'} |",
        f"| Daily TP | {'$'+str(int(DAILY_TP)) if DAILY_TP is not None else 'off'} |",
        f"| Dynamic SL | {'on $'+str(int(DYNAMIC_SL_AMOUNT)) if ENABLE_DYNAMIC_SL else 'off'} |",
        f"| EOM SL | {'on $'+str(int(EOM_SL_AMOUNT)) if ENABLE_EOM_SL else 'off'} |",
        f"| SL gap re-entry | {'on '+str(SL_GAP_MINUTES)+'min' if ENABLE_SL_GAP_REENTRY else 'off'} |",
        f"| VIX max filter | {VIX_MAX_FILTER if VIX_MAX_FILTER is not None else 'off'} |",
        f"| VIX min filter | {VIX_MIN_FILTER if VIX_MIN_FILTER is not None else 'off'} |",
        f"| Econ filter | {'on' if ENABLE_ECON_FILTER else 'off'} |",
        f"| Calendar filter | {'on ('+', '.join(sorted(CALENDAR_FILTER_EVENTS))+')' if ENABLE_CALENDAR_FILTER else 'off'} |",
        ...
        f"| Econ+VIX skip | {'on ('+str(len(_cfg.ECON_VIX_SKIP_RULES))+' rules)' if _cfg.ENABLE_ECON_VIX_SKIP else 'off'} |",
